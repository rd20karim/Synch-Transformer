...
class dataset_class(Dataset):
    def __init__(self, path, train=False, test=False, val=False, path_txt=None, min_freq=1, filter_data=False,
                 joint_angles=False, multiple_references=False,random_state=11):
        ...
        if not splits:
            ...
            logging.info(f"Use official split")
        ...
        dw_factor = 2
        def remove_samples(data, p=1000):
        ...
        if not multiple_references:
            ...
            Max_Src_Len = 300 # Just for training in inference
            logging.info('Only for training : Select samples with length less %d' % Max_Src_Len)
            [self.X_train, self.y_train], [self.X_val, self.y_val], [self.X_test, self.y_test] = [remove_samples(data,p=Max_Src_Len)
                                                                                                  for data in [[self.X_train,self.y_train],[self.X_val, self.y_val],[self.X_test,self.y_test]]]
        ...
        self.Sets = [self.X_train, self.y_train, self.X_val, self.y_val, self.X_test, self.y_test] \
            if not self.multiple_references else [self.X_ref_train, self.y_ref_train, self.X_ref_val,
                                                  self.y_ref_val,self.X_ref_test, self.y_ref_test]

    def __getitem__(self, item):
        if self.train_:
            return self.Sets[0][item], self.Sets[1][item]
        elif self.val_:
            return self.Sets[2][item], self.Sets[3][item]
        elif self.test_:
