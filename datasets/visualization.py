...
def decode_predictions_and_compute_bleu_score(output,trg,vocab_obj,num_grams=4,batch_first=False,multiple_references = False):
        ...
        for k in range(numeric_predictions.size(1)):
            predicted_sentences += [_dec_numeric_sentence(numeric_predictions[:,k].cpu().numpy().tolist(),remove_sos_eos=True).replace('_',' ').split() ]

        if multiple_references:
            input_sentences = [[_dec_numeric_sentence(ref,remove_sos_eos=True).replace('_',' ').split() for ref in refs] for refs in trg ]
            
        else:
            input_sentences = [[_dec_numeric_sentence(inp_.tolist(),remove_sos_eos=True).replace('_',' ').split()] for inp_ in trg.T.cpu().numpy() ]
