...
class dataset_class(Dataset):
    def __init__(self, path, train=False, test=False,val=False, path_txt=None,min_freq=1,filter_data=False,
        ...
        for desc in self.corrected_sentences:
            # # TODO remove later
            # desc = " ".join(self.lang.merge_sentence(desc.split(),self.lang.counts))
            # # TODO End remove
            # # print(desc)
        ...
        def sort_wlen(sequence1, sequence2):
            ...
            lengths = np.array([len(s) for s in sequence1])
            # Get sorted indices based on lengths
            id_lg = np.argsort(lengths)
            # Reorder both sequences using advanced NumPy indexing
            # Ensure sequence1 and sequence2 are NumPy arrays for efficient indexing
            sq1 = np.array(sequence1)[id_lg]
            sq2 = np.array(sequence2)[id_lg]
       ...
       # TODO WAS CHANGED DENSE DOWN SAMPLING
        ...
        # delete from memory usage
        del self.poses,self.sentences,self.corrected_sentences
    ...
    def __getitem__(self, item):
        if self.train_:
            ...
            return self.Sets[4][item], self.Sets[5][item]
    ...
    def shift_poses(self):
        ...
        logging.info(f"Mean: {mx},{my},{mz}")
        logging.info(f"Std: {sx},{sy},{sz}")
