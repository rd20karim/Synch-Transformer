...
class vocabulary():
    ...
    def token_correction(self,ask_user=False):
        ...
        for token in replace.keys():
            for i in range(len(self.sentences)):
                print("%d/%d\r"%(i,len(self.sentences)),end="")
                self.sentences[i] = self.sentences[i].lower().replace(token,replace[token])

        spell = SpellChecker()
        shift = 0
        # CREATE FILE TO SAVE CORRECTED DESCRIPTIONS
        with open("sentences_corrections"+".csv", mode="w") as fa: pass
        for idx, desc in enumerate(self.sentences):
            desc = str(desc)
            # PRE-PROCESSING
            ...
            if len(list(misppelled)) != 0:
                for token in list(misppelled):
                    ...
                    if not ask_user or not user_correction: # ask_user for correction
                        try:
    ...
    def build_vocabulary(self, min_freq=1):
        ...
        with open("./dataset_vocab.txt", mode='w',encoding='utf8') as fw:
    ...
    def decode_numeric_sentence(self,list_int,remove_sos_eos=False,ignore_pad = False):
        ...
        if remove_sos_eos:
            ...
            try :
                ...
                # WHEN THE EOS TOKEN IS NOT GENERATED IN THE PREDICTION
    ...
    def language_tokenizer(self,sentence):
        ...
        return [str(token) for token in self.spacy_eng.tokenizer(sentence) if not token.is_space]