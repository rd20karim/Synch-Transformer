...
def beam_decode(self,target_tensor, decoder_hiddens, encoder_outputs=None):
    ...
    # ------- Set a good initialization --------
    # Initialize p_t with index of minimal velocity above 0.05
    v_mask = torch.norm(self.x[:, :, 1:] - self.x[:, :, :-1], 2, dim=-1) > 0.05
    ...
    for idx in range(B):
        ...
        self.pt = torch.tensor([torch.argmax(v_mask.int(), dim=0)[idx]],device=self.device) #torch.zeros((1,), device=self.device)
        self.dec.pt = self.pt
        ...
        #for idx in range(trg_len-1):
            ...
            for new_k in range(beam_width):
                ...
                node = BeamSearchNode(hiddenstate=decoder_hidden, previousNode=n, wordId=decoded_t.item(),
                                      ...
                                      att_position=int(self.dec.pt[0][0][0]))
